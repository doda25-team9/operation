...
Use the `curl` to send requests, for instance:
```
for i in {1..100}; do
...
```
The system is configured with a **90/10 Canary Split** (v1/v2) and a **100% Shadow Mirror** to v3.
...
| :--- | :--- | :--- | :--- |
| **v1** | Stable | **90** | Handles 90% of live traffic. |
| **v2** | Canary | **10** | Handles 10% of live traffic. |
| **v3** | Shadow | **100** | **Mirrors 100% of traffic.** |

![v1](../images/image.png)
![v2](../images/image2.png)
![v3](../images/image3.png)

The total live traffic was `90 (v1) + 10 (v2) = 100`. The shadow instance (v3) processed `100` requests. The counts match, proving that **v3 receives a copy of all traffic** without affecting the user response.
...
**v3 (Shadow) - Model v0.2.0:**
```text
model_prediction_latency_seconds_bucket{le="0.005",version="v0.2.0"} 98.0
model_prediction_latency_seconds_count{version="v0.2.0"} 100.0
```
98 out of 100 requests (98%) on the new model were processed in under **5ms**.

**v1 (Stable) - Model v0.1.0:**
```text
model_prediction_latency_seconds_bucket{le="0.005",version="v0.1.0"} 0.0
model_prediction_latency_seconds_bucket{le="0.025",version="v0.1.0"} 89.0
```
The stable model is significantly slower; 0 requests were under 5ms, and the majority took up to 25ms (only 14 were under 10ms).

### 3. Final Conclusion

The metrics prove that the Shadow Model (v0.2.0) is significantly faster (sub-5ms) than the Stable Model (v0.1.0).
This data was collected from real production traffic mirrored to v3, but users only received responses from v1/v2.